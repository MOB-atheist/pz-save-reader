...
import { decodePzBuffer } from "./decode-pz-buffer.js";
...
const SNAPSHOT_DIR = path.join(runtimeConfig.getAppBaseDir(), "data", "snapshots");
const SOURCE_DIR = path.join(runtimeConfig.getAppBaseDir(), "data", "source");
...
/**
 * Create and return the Express application (no listen). Used by both CLI and Electron.
...
/**
 * Start the server: ensure cache ready, then listen on port.
 * @param {number} port
